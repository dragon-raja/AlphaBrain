...
import scripts.dsol_paper1.runtime.run_matched_view_accel_v1 as scorer
...
def test_code_identities_point_to_real_accel_implementation():
    ...
    assert any(path.endswith("AlphaBrain/research/dsol/metrics/accel_inference.py") for path in paths)
    assert not any(path.endswith("scripts/cabi_vla/accel_inference.py") for path in paths)
    assert any(path.endswith("accel.py") for path in paths)
