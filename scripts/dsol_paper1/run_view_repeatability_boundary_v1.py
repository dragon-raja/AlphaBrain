...
from build_matched_view_landscape_protocol_v1 import source_identity, write_new_json
...
def main():
    ...
    write_new_json(ROOT/'boundary-entry.json', {'gate_process': process_identity(os.getpid()),
        ...
        'source': source_identity(Path(__file__)), 'driver': source_identity(control.REPO/'scripts/dsol_paper1/run_view_repeatability_probe_v1.py')})
    ...
    try:
        ...
        with (ROOT/'probe.log').open('a') as stream:
            probe=subprocess.Popen(['timeout','--signal=TERM','--kill-after=30s','1800s',sys.executable,
                str(control.REPO/'scripts/dsol_paper1/run_view_repeatability_probe_v1.py')],stdout=stream,stderr=subprocess.STDOUT,start_new_session=True)
