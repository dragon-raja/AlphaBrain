...
def test_current_runtime_and_active_controller_sources_are_byte_identical():
...
def test_every_original_source_is_recoverable_and_moved_path_is_absent():
    ...
    assert len(list((ROOT / "scripts/dsol_paper1").glob("*.py"))) + len(list((ROOT / "scripts/dsol_paper1").glob("*.sh"))) == 37
