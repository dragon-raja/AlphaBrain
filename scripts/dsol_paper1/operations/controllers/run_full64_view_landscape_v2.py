...
RUNNER=REPO/'scripts/dsol_paper1/operations/launchers/run_dsol_libero_hdf5_closed_loop_eval.sh'
AUDITOR=REPO/'scripts/dsol_paper1/diagnostics/audit_statewise_view_oracle_v2_run.py'
ACTIVE=[]
STOPPED=False
...
def preflight():
    from scripts.dsol_paper1.runtime.shared_runtime_paths import shared_code_paths
    ...
    runner_files=[REPO/'scripts/cabi_vla/serve_alphabrain_pi05_websocket.py',REPO/'scripts/cabi_vla/serve_openpi_deterministic.py',
        REPO/'scripts/dsol_paper1/runtime/evaluate_dsol_libero_hdf5_views.py',REPO/'scripts/dsol_paper1/analysis/summarize_dsol_libero_hdf5_closed_loop.py',RUNNER]
        ...
        try:
            ...
            for gpu in range(8):
                ...
                proc=subprocess.Popen(['timeout','--signal=TERM','--kill-after=120s','7200s',sys.executable,
                    str(REPO/'scripts/dsol_paper1/runtime/run_matched_view_accel_v1.py'),'--manifest',str(manifest_path),'--num-shards','8','--shard-index',str(gpu),'--device','cuda:0'],
...
def execute(release):
    ...
    command([sys.executable,REPO/'scripts/dsol_paper1/operations/controllers/finalize_full64_view_landscape_v2.py'],
