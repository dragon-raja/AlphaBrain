...
物理状态的候选间共享完全相同的 `x0`，保存 `accel_2...10`，再检验
`argmin_v accel_3(v)` 能否选择闭环更好的视角。该假设按一条连续证据链裁决：

1. **行为价值**：候选池中是否存在更好的闭环视角，Accel 是否兑现该空间；
2. **测量可靠性**：改变 flow noise 后，97 视角排序是否可重复；
3. **支持域归因**：低-Accel 视角来自规范位姿、训练精确位姿还是同范围留出位姿；
4. **任务信息对齐**：能否偏好最高可见性视角，同时拒绝 Blind、Look-away 与黑相机；
5. **关系可泛化性**：上述偏好是否跨任务和操作阶段保持同一方向。

任何一环失败，都不能把 Accel 直接部署为视角价值选择器。
...
Broad practical 的任一候选成功率比 Canonical 高 28.6pp，但 Accel 所选成功率没有提高。四模型的
...
- 六个噪声下精确 Top-1 全同仅 `2.1%–4.2%`；
- ensemble 第一、二名相对间隔仅 `1.68%–3.69%`。

因此单次 `argmin Accel` 主要受 flow noise 影响；后续只能使用多噪声 ensemble 作为诊断。
...
Canonical attractor，但低-Accel 兼容域并未锁死在 64 个训练精确位姿。Broad practical 与
State-matched 的留出位姿富集分别为 `1.23×`、`1.07×`，而 Paired-FM 与 consistency 为
`0.82×`、`0.76×`。这支持 Broad practical 在同一宽范围内的兼容性泛化，但不等于任务价值识别。

#### Gate D：任务信息对齐与坏观测拒绝

- 全黑相机平均位于第 `101/104`，Look-away 位于第 `86–92`，说明 Accel 能稳定排斥明显无效观测；
- 最高可见性视角平均仅位于第 `46–51`；
- `rank(strong-info)-rank(matched-control)` 在四模型上为
  `+2.78、+5.67、+4.73、-0.48`，负值才表示信息视角更受偏好。

因此 Accel 能测量明显失效或生成不顺滑，但没有形成“可见任务信息更多即 Accel 更低”的关系。

#### Gate E：跨任务与阶段一致性

按 8 个任务和四个轨迹阶段拆分后，信息相对控制的排名差反复改变符号。总体均值会把相反方向
抵消成接近零，不能写成统一模型规律。后续闭环 shortlist 必须按任务均衡抽样，不能只挑 Accel
信号最强的状态。
...
因此保留 Accel 作为 6-noise ensemble 的模型兼容域和坏观测诊断器，停止把单次
`argmin Accel` 直接部署为主动视角价值函数。该结论不否定原文将 Accel 用作 uncertainty proxy
