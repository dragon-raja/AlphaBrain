...
PYTHONPATH="$REPO_ROOT/scripts/dsol_paper1:$REPO_ROOT" \
  /alphabrain/.venv/bin/python \
    "$REPO_ROOT/reports/paper1/historical/build_view_value_expectation_final_report.py" \
      --root "$ROOT" --output-dir "$OUTPUT"

WANDB_MODE=${WANDB_MODE:-offline} \
  /alphabrain/.venv/bin/python \
    "$REPO_ROOT/scripts/dsol_paper1/operations/controllers/log_view_value_expectation_wandb.py" \
