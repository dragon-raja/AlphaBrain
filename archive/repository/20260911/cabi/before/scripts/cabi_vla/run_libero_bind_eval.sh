...
OUTPUT_ROOT=${CABI_EVAL_OUTPUT_ROOT:-/share/longjunyu/cabi-vla/evaluations}
SPLIT=${CABI_EVAL_SPLIT:-val}
...
CHECKPOINT=${1:?usage: run_libero_bind_eval.sh CHECKPOINT RUN_NAME GPU_ID}
RUN_NAME=${2:?usage: run_libero_bind_eval.sh CHECKPOINT RUN_NAME GPU_ID}
GPU_ID=${3:?usage: run_libero_bind_eval.sh CHECKPOINT RUN_NAME GPU_ID}

if [[ ! "$RUN_NAME" =~ ^[A-Za-z0-9_.-]+$ ]]; then
  echo "RUN_NAME may only contain letters, numbers, dots, underscores, and dashes" >&2
  exit 2
fi
...
if [[ ${#horizon_args[@]} -eq 0 ]]; then
  echo "CABI_EVAL_HORIZONS must contain at least one K" >&2
  exit 2
fi
...
OUTPUT="$RUN_DIR/closed_loop_${SPLIT}.json"
PARTIAL="$RUN_DIR/closed_loop_${SPLIT}.partial.json"
SOCKET="/tmp/cabi-bind-${RUN_NAME}-$$.sock"
...
CUDA_VISIBLE_DEVICES="$GPU_ID" \
PYTHONDONTWRITEBYTECODE=1 \
"$SIM_PYTHON" scripts/cabi_vla/evaluate_libero_bind_closed_loop.py \
