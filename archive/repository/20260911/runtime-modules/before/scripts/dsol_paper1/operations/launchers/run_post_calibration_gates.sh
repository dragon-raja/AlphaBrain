...
"/alphabrain/.venv/bin/python" "$REPO_ROOT/scripts/dsol_paper1/analysis/summarize_dsol_calibration.py" \
...
if [[ ! -s "$PAIR_SMOKE_RUN/metrics.jsonl" ]] || \
  ...
  DSOL_CALIBRATION=0 DSOL_SKIP_FINAL_SAVE=1 WANDB_MODE=offline \
    "$REPO_ROOT/scripts/dsol_paper1/training/run_libero_pair_train.sh" \
