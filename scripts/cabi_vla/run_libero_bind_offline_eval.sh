...
OUTPUT_ROOT=${CABI_OFFLINE_OUTPUT_ROOT:-/share/longjunyu/cabi-vla/offline-evaluations}
SEEDS=${CABI_OFFLINE_SEEDS:-20260722}
MAX_TETRADS=${CABI_OFFLINE_MAX_TETRADS:-}
SERVER_TIMEOUT=${CABI_POLICY_SERVER_TIMEOUT:-600}

CHECKPOINT=${1:?usage: run_libero_bind_offline_eval.sh CHECKPOINT RUN_NAME GPU_ID}
RUN_NAME=${2:?usage: run_libero_bind_offline_eval.sh CHECKPOINT RUN_NAME GPU_ID}
GPU_ID=${3:?usage: run_libero_bind_offline_eval.sh CHECKPOINT RUN_NAME GPU_ID}
...
OUTPUT="$RUN_DIR/offline.json"
SOCKET="/tmp/cabi-offline-${RUN_NAME}-$$.sock"
...
max_args=()
if [[ -n "$MAX_TETRADS" ]]; then
  max_args=(--max-tetrads "$MAX_TETRADS")
fi
"$PYTHON" scripts/cabi_vla/evaluate_libero_bind_offline.py \
