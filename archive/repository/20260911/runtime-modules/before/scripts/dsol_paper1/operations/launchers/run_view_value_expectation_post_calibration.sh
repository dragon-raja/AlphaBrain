...
run_primary_analysis() {
  "$POLICY_PYTHON" "$REPO_ROOT/scripts/dsol_paper1/analysis/analyze_view_value_expectation_heldout.py" \
...
"$POLICY_PYTHON" "$REPO_ROOT/scripts/dsol_paper1/protocols/build_view_value_expectation_heldout_protocols.py" \
...
if jq -e '.activate_bank_F == true' "$HELDOUT_ROOT/analysis-primary/reserve-decision.json" >/dev/null; then
  for seed in 41 42 43; do
    "$POLICY_PYTHON" "$REPO_ROOT/scripts/dsol_paper1/protocols/build_view_value_expectation_reserve_protocol.py" \
