...
if [[ "$RENDER_SOURCE_DIR" == "$OUTPUT_DIR" ]]; then
  PYTHONPATH="$REPO_ROOT:/projects/openpi/packages/openpi-client/src:$REPO_ROOT/scripts/cabi_vla:$REPO_ROOT/scripts/dsol_paper1" \
    "$SIM_PYTHON" "$REPO_ROOT/scripts/dsol_paper1/operations/controllers/run_constructed_accel_bank.py" \
      --stage render \
      --protocol "$PROTOCOL" \
      --runtime "$RUNTIME" \
...
PYTHONPATH="$REPO_ROOT:/projects/openpi/src:/projects/openpi/packages/openpi-client/src" \
  "$MODEL_PYTHON" "$REPO_ROOT/scripts/dsol_paper1/operations/controllers/run_constructed_accel_bank.py" \
    --stage rank \
    --protocol "$PROTOCOL" \
