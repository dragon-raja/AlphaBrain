...
报告范围：主体保留原完成时点的实验叙述。2026-08-05 补充审计确认，后续
“场景线索 × 腕部相机”三随机种子因子实验已经完成；其结果见第 8 节更新。
...
3. **在相同多视角数据上，再显式告诉 Pi0.5 相机内外参是否更好？目前没有。**
   ...
   95% CI `[-7.03,+4.43] pp`。进一步固定 RGB、只替换 ray map 时，
   KYC 的动作块 RMS 变化仅为 `0.00071`，说明当前策略几乎没有使用
   ray 中随相机位姿变化的部分。
...
#### Control 的 ray 是否与评测相机匹配

设默认相机位姿为 `q0`，当前图像真实位姿为 `q`：

| 方法 | 策略看到的外部 RGB | 相机分支收到的 ray | 非默认视角是否匹配 |
|---|---|---|---|
| PoseAug-Control | `I(q)` | 永远是 `R(q0)` | 否 |
| KYC | `I(q)` | 当前真实位姿的 `R(q)` | 是 |

因此 Control 在除默认视角外确实收到故意错配的 ray，而 KYC 会在每次策略
推理时根据当前评测相机内外参重新生成匹配 ray。这里的“每次推理更新”不能误解为
相机在 episode 内连续运动：当前实验是在 episode 开始前设置一个相机位姿，
此后保持固定。它检验的是已知标定下的跨 episode 位姿变化，不是动态移动相机。
...
| 方法 | 训练时看到的外部相机 | 评测时使用的外部相机 |
|---|---|---|
| Base | 只有默认固定视角 | 与所有方法相同的 13 位姿固定测试网格 |
| PoseAug-RGB | 从训练范围生成的随机多视角 RGB | 同一个 13 位姿固定测试网格 |
| PoseAug-Control | 同一批随机多视角 RGB，另加固定默认 rays | 同一个 13 位姿固定测试网格 |
| KYC | 同一批随机多视角 RGB，另加每张图的真实 rays | 同一个 13 位姿固定测试网格 |

因此评测不是“每次运行临时随机抽一个视角”。它预先冻结：

- 默认视角 1 个；
- 水平角 `{-90°,-60°,+60°,+90°}`；
- 俯仰角 `{-32°,-25°,+25°,+32°}`；
- 距离 `{0.75×,0.90×,1.25×,1.40×}`；
- 合计 13 个互不重复的相机位姿。

每个方法、seed、任务和初始状态都使用完全相同的位姿，因此曲线上的差异不能
归因于某个方法随机抽到了更容易的相机。浅绿色范围内的点检验训练支持域内
泛化，绿色范围外的点检验向未见更极端相机位姿的外推。
...
如果所说的“第三个图”指第三排黑线，它不是成功率，也不是训练结果，而是纯粹
检查输入图像：

- 水平角一列为 `95% 至 100%`，说明即使水平移动很大，物体基本仍在画面中；
- 俯仰角 `+32°` 降至 `55%`，说明一部分任务已经发生明显裁切或目标过小；
- 距离 `0.75×` 为 `0%`、`0.90×` 为 `20%`，说明相机太近会把任务物体裁出
  外部图像；
- 距离 `1.0×` 及更远为 `100%`，说明物体仍满足最低像素门槛。

如果所说的“第三个图”指第三列，它表示相机距离：`1.0×` 是默认约
`0.908 m`，`0.75×` 约 `0.681 m`，`0.90×` 约 `0.817 m`，
`1.25×` 约 `1.135 m`，`1.40×` 约 `1.271 m`。
...
### 5.3 为什么错误 ray 的 Control 仍能工作

这不是“Control 用错误几何完成了正确三维反投影”。当前相机分支是一个通过
动作损失学习的软条件分支，不会强制执行几何投影。固定默认 ray 在每张图上虽然
相同，但在不同像素处仍有不同数值，因此它可以退化成固定空间位置模板或可学习
偏置。模型也可以把这条分支的权重压低，主要依赖 RGB 和腕部图像。

多视角训练在当前仿真中尤其有效，原因是：

1. 外部相机只改变渲染，不改变机器人状态、物体状态或专家动作。不同视角对应
   完全一致的机器人坐标系动作标签，是近乎理想的无噪声、标签保持增强。
2. 训练范围内有 `138` 个不同相机 shard、`22,464` 个训练 records；模型反复
   看到“外观投影改变，但正确动作不变”，直接受到学习视角不变表示的监督。
3. LIBERO 桌面、机器人和背景关系固定，RGB 本身可以泄漏相机位姿；已有线性
   probe 在固定场景中达到 `100%` 相机位姿分类准确率。
4. 腕部相机不受外部相机位姿干预，始终提供机器人中心的稳定视觉参照。
5. 当前动作在机器人控制坐标系中表达，而不是相机坐标系。模型只要从多视角
   图像恢复出足够的任务关系，不一定需要显式相机标定。

所以多视角增强和 KYC 提供的不是同一种信息：

- 多视角增强直接告诉模型“这些外观变化都应映射到同一类动作”；
- ray map 只告诉模型“每个像素对应哪条世界射线”，既不提供深度，也不保证
  模型会把 RGB 物体特征绑定到该射线。

当 RGB、背景和腕部图像已经足以降低动作损失时，使用 ray 反而是一条更难的
学习路径。KYC 可以通过把 pose-dependent ray 信息近似忽略，收敛到与 Control
类似的解。

这个解释有直接干预证据。对已训练 KYC 固定同一 RGB、腕部图像和扩散噪声，
只把正确 ray 换成默认或其他位姿的 ray：

| 干预 | 动作块 RMS 差异 | 余弦相似度 |
|---|---:|---:|
| 正确 ray → 默认 ray | `0.000710` | `0.9999988` |
| 正确 ray → 其他位姿 ray | `0.000953` | `0.9999983` |

也就是说，策略输出对 ray 位姿几乎不敏感。更准确的结论是“当前 KYC 适配没有
学会使用精确相机几何”，而不是“错误几何也同样正确”。Control 比 KYC 高
`1.16 pp` 的区间跨过 0，应解释为两者无稳定差异，不能解释为错误 ray 更优。

当前证据仍不能区分两个更深层原因：

- 在固定场景和腕部相机开启时，精确标定本来就是冗余信息；
- Pi0.5 的冻结视觉编码器与后融合方式不利于学习 ray 和 RGB 的几何对应。

正在运行的“场景线索随机化 × 腕部相机开关”因子实验用于检验第一项。当前尚未
完成 episode 内连续移动相机的评测，因此不能把本结论外推到实时运动相机。
...
每个单元都比较 PoseAug-Control 与 KYC，三随机种子确认结果为：

| 场景线索 | 腕部输入 | Control | KYC | 解释 |
|---|---|---:|---:|---|
| 固定 | 保留 | `34.34%` | `31.03%` | 未观察到 KYC 增量 |
| 随机化 | 保留 | `30.03%` | `29.60%` | 未观察到 KYC 增量 |
| 固定 | 屏蔽 | `0.29%` | `0.14%` | 两个基线均失效 |
| 随机化 | 屏蔽 | `1.15%` | `0.72%` | 两个基线均失效 |

因此，场景线索随机化本身没有显露出被固定背景掩盖的 KYC 收益；但腕部关闭
条件无法回答因果问题，因为 Pi0.5 在该训练配置下已经基本不会完成任务。机器
结果为
...
4. 腕部关闭单元基线失效，因此仍不能用该因子实验单独量化腕部相机对 KYC
   增量的遮蔽程度；但保留腕部时，固定与线索随机场景都没有显示 KYC 增益。
...
6. **为什么不同与下一步**：固定背景会泄漏相机位置，但线索随机化后 KYC 仍
   未增益；腕部关闭基线失效。下一步转向有效 Pi0.5 基线上的相机×背景联合
   域外和严格留组合验证。
