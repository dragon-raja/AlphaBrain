...
2026-08-27 首轮 200-episode pilot 进一步发现 spatial 任务的 5 个晚期状态在所有条件（包括全黑相机）下均于 1–4 步成功，说明旧筛选没有排除初态已满足任务条件的状态。该轮仅作为协议审计，不作为科学结果。扫描器与筛选器现已增加 `initial_task_success == false` 的前置门，必须重新筛选和评测。
