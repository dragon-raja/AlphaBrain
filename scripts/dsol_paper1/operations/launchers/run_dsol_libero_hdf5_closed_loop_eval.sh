...
ANALYZER=${ANALYZER:-$REPO_ROOT/scripts/dsol_paper1/analysis/summarize_dsol_libero_hdf5_closed_loop.py}
...
if [[ -d "$REPO_ROOT/scripts/vla_shared" ]]; then
  ...
  shared_code_files+=("$REPO_ROOT/scripts/dsol_paper1/runtime/shared_runtime_paths.py")
fi
code_sha256=$(sha256sum \
  ...
  "$REPO_ROOT/scripts/dsol_paper1/runtime/evaluate_dsol_libero_hdf5_views.py" \
  "$ANALYZER" \
...
for ((shard=0; shard<EVAL_WORKER_COUNT; shard++)); do
  ...
  PYTHONPATH="$REPO_ROOT:/projects/openpi/packages/openpi-client/src:$REPO_ROOT/scripts/cabi_vla:$REPO_ROOT/scripts/dsol_paper1" \
    "$SIM_PYTHON" "$REPO_ROOT/scripts/dsol_paper1/runtime/evaluate_dsol_libero_hdf5_views.py" \
...
from scripts.dsol_paper1.runtime.evaluate_dsol_libero_hdf5_views import (
    protocol_spec_count,
    selected_spec_count,
)
